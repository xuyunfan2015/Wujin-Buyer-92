...
- (void)viewDidLoad
{
    ...
    [self layoutNavigationBarWithString:self.nickName];
...
#pragma mark - 以下三行代码必须写，注册为SDK的ChatManager的delegate
   ...
   // [self setupBarButtonItem];
    ...
    if (self.productInfo) {
        ...
        [self sendTextMessage:self.productInfo];
        
    }
    
    if (self.image) {
        
        [self sendImageMessage:self.image];
    }
}
...
#pragma mark - 以下第一行代码必须写，将self从ChatManager的代理中移除
    [[EaseMob sharedInstance].chatManager removeDelegate:self];
...
- (NSURL *)convert2Mp4:(NSURL *)movUrl {
    ...
    if ([compatiblePresets containsObject:AVAssetExportPresetHighestQuality]) {
        ...
        NSInteger timeout = dispatch_semaphore_wait(wait, DISPATCH_TIME_FOREVER);
        if (timeout) {
            NSLog(@"timeout.");
        }
        if (wait) {
...
- (UITableView *)tableView
{
    ...
    if (_tableView == nil) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, NAV_HEIGHT, self.view.frame.size.width, self.view.frame.size.height - self.chatToolBar.frame.size.height - NAV_HEIGHT) style:UITableViewStylePlain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row < [self.dataSource count]) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            NSString * loginUsername = [UserInfo sharedUserInfo].userName;
            
            NSString * headPhoto = nil;
            
            if ([AppUtil isNotNull:[UserInfo sharedUserInfo].image]) {
                
                headPhoto  = [CommentRequest getCompleteImageURLStringWithSubURL:[UserInfo sharedUserInfo].image];
                
            }
            
            if (model.isSender) {///是本地的自己
                
                if (headPhoto) {//存在
                    
                    model.headImageURL = [NSURL URLWithString:headPhoto];
                   
                 //  cell.headImageView.userInteractionEnabled = YES;
                    //cell.headImageView.delegate = self;
                   // cell.headImageView.tag = 801;
            ...
            if ([model.username isEqualToString:loginUsername]) {///是本地的自己
                
                cell.headImageView.userInteractionEnabled = YES;
                cell.headImageView.delegate = self;
                cell.headImageView.tag = 8001;
            ...
            [chatManager asyncFetchMessage:model.message progress:nil completion:^(EMMessage *aMessage, EMError *error) {
                ...
                if (!error) {
                    ...
                    if (localPath && localPath.length > 0) {
                        ...
                        [weakSelf openImageSetWithUrls:@[url] initImageIndex:0];
...
- (void)didChangeFrameToHeight:(CGFloat)toHeight
{
    [UIView animateWithDuration:0.3 animations:^{
        ...
        rect.size.height = self.view.frame.size.height - toHeight - NAV_HEIGHT;
...
-(void)sendTextMessage:(NSString *)textMessage
{
   ...
   // }
  //  EMMessage *tempMessage = [ChatSendHelper sendTextMessageWithString:textMessage toUsername:@"15001842839" isChatGroup:_isChatGroup requireEncryption:NO];
 //   [self addChatDataToMessage:tempMessage];
}
...
#pragma mark - WTUrlImaeViewDelegate

/////点击头像跳转
- (void) URLImageViewDidClicked : (WTURLImageView*)imageView {
    
    if (imageView.tag == 8001) {//自己的  跳转还没做
        
        ///个人资料
//        UIStoryboard * board = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
        
       /* UserInfoViewController * userinfoViewController = [board instantiateViewControllerWithIdentifier:@"UserInfoViewController"];
        
        [self.navigationController pushViewController:userinfoViewController animated:YES];*/

        
    }else if(imageView.tag == 8002) {//跳转还没做 故意不做的
        
        /*UIStoryboard * board = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
        
        OtherUserViewController * otherUserViewController = [board instantiateViewControllerWithIdentifier:@"OtherUserViewController"];
        otherUserViewController.userID = self.userID;
        otherUserViewController.nickName = self.nickName;
