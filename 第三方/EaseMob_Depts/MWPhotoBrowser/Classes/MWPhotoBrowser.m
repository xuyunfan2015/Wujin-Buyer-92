...
- (void)dealloc {
    ...
    [[SDImageCache sharedImageCache] clearMemory]; // clear
    [_photos release];
...
- (void)viewDidLoad {
    ...
    [_backButton setFrame:CGRectMake(10, 25 , 30, 24)];
    [_backButton setBackgroundImage:[UIImage imageNamed:@"MWPhotoBrowser.bundle/images/UIBarButtonItemArrowLeft.png"] forState:UIControlStateNormal];
    [_backButton addTarget:self action:@selector(backAction) forControlEvents:UIControlEventTouchUpInside];
    
    _myTitle = [[UILabel alloc] initWithFrame:CGRectMake(70, 25, 180, 24)];
    _myTitle.backgroundColor = [UIColor clearColor];
    _myTitle.font = [UIFont fontWithName:@"Bold" size:25];
    _myTitle.textAlignment = NSTextAlignmentCenter;
    _myTitle.textColor = [UIColor whiteColor];
    
    _topBar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, 0, K_UIMAINSCREEN_WIDTH, 64)];
    _topBar.tintColor = nil;
    if ([[UIToolbar class] respondsToSelector:@selector(appearance)]) {
        [_topBar setBackgroundImage:nil forToolbarPosition:UIToolbarPositionAny barMetrics:UIBarMetricsDefault];
        [_topBar setBackgroundImage:nil forToolbarPosition:UIToolbarPositionAny barMetrics:UIBarMetricsLandscapePhone];
    }
    _topBar.barStyle = UIBarStyleBlackTranslucent;
    _topBar.autoresizingMask = UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleWidth;
    
    [_topBar addSubview:_backButton];
    [_topBar addSubview:_myTitle];
...
- (void)updateNavigation {
	...
	if ([self numberOfPhotos] > 1) {
		_myTitle.text = [NSString stringWithFormat:@"%i %@ %i", _currentPageIndex+1, NSLocalizedString(@"of", @"Used in the context: 'Showing 1 of 3 items'"), [self numberOfPhotos]];
	} else {
		_myTitle.text = nil;
	}
...
- (void)setControlsHidden:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    ...
    if (self.wantsFullScreenLayout) {
       ...
       // CGRect navBarFrame = _myview.frame;
       // navBarFrame.origin.y = statusBarHeight;
       // _myview.frame = navBarFrame;
        ...
        if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
            ...
            if ([MFMailComposeViewController canSendMail]) {
                ...
                self.actionsSheet = [[[UIActionSheet alloc] initWithTitle:nil delegate:self
                 ...
                 otherButtonTitles:@"保存",@"复制", nil] autorelease];
