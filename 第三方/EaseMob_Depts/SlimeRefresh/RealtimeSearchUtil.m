...
- (void)searchBegin:(NSString *)string
{
    ...
    dispatch_async(self.searchQueue, ^{
        if (string.length == 0) {
            weakSelf.resultBlock([weakSelf.source valueForKey:@"dataSource"]);
        }
        else{
            NSMutableArray *results = [NSMutableArray array];
            NSString *subStr = [string lowercaseString];
            
            NSArray * dataSource = [weakSelf.source valueForKey:@"dataSource"];
            NSArray * userInfo = [weakSelf.source valueForKey:@"userInfo"];
            
            for (int index = 0; index < [dataSource count]; index ++) {
                
                BOOL isAdd = NO;
                
                id object = dataSource[index];
                
                 NSDictionary * userIn = [userInfo valueForKey:[object valueForKey:@"chatter"]];///获取userIn
                
                if ([AppUtil isNotNull:[userIn valueForKey:@"name"]]) {
                    
                    NSString * nickName = [userIn valueForKey:@"name"];
                    
                    nickName = [nickName lowercaseString];
                    
                    if([nickName rangeOfString:subStr].location != NSNotFound)
                    {
                        isAdd = YES;
...
- (void)realtimeSearchWithSource:(id)source searchText:(NSString *)searchText collationStringSelector:(SEL)selector resultBlock:(RealtimeSearchResultsBlock)resultBlock
{
    if (!source || !searchText || !resultBlock) {
        _resultBlock([source valueForKey:@"dataSource"]);
        return;
    }
