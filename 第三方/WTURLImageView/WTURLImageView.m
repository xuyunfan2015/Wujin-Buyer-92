...
- (void)setURL:(NSString *)url defaultImage:(UIImage *)image {
    ...
    if (!url || [url isKindOfClass:[NSNull class]]) {
        ...
        [self setURL:[NSURL URLWithString:url] fillType:UIImageResizeFillTypeIgnoreAspectRatio options:WTURLImageViewOptionTransitionPerspectiveDissolve placeholderImage:image failedImage:image diskCacheTimeoutInterval:100 * 24 * 60 * 60 * 1000];
    }
}
...
- (void)setURLWithURLString:(NSString *)urlString {
    
   /* ////不被拉伸 是 fitIn
    
    [self setURLRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]] fillType:UIImageResizeFillTypeFillIn options:WTURLImageViewOptionDontUseConnectionCache placeholderImage:TTImage(@"avatar_default") failedImage:TTImage(@"avatar_default") diskCacheTimeoutInterval:10 success:^(NSURLRequest *request, NSHTTPURLResponse *response, UIImage *image) {
